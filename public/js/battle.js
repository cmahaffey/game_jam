...
BedJam.Battle.prototype.powersList = function powersList() {
  ...
  this.powerList=[this.hurt, this.heal, this.winks, this.beBrave, this.tantrum]
  // you can set them to display: none and as char levels, have them display
    // maybe a boolean once unlocked
  this.powersUl.append( //hopefully for loops work in here
    for (i=0;i<=BedJam.girl.abilities.length; i++){
    this.powerList[i];
  });
  ...
  this.hurt.on('click', function(){
    //copiable for every other ability
    //anything not scope.enemy MAY need to be BedJam.girl
    scope.clearPowers;
    scope.statusMessage = BedJam.girl.battle(scope.enemy);

    if (scope.statusMessage) {
      scope.damageDealt = scope.statusMessage + "<br>" + BedJam.girl.critCheck(girl.abilities[0].spell(BedJam.girl,scope.enemy);
    } else {
      scope.damageDealt = BedJam.girl.critCheck(scope.enemy);
    }

    if (BedJam.girl.win(scope.enemy)) {
      scope.expMessage = BedJam.girl.win(scope.enemy) + "<br>" + (girl.abilities[0].spell(BedJam.girl,scope.enemy));
      scope.battleResults(scope.damageDealt);
...
BedJam.Battle.prototype.runAway = function runAway() {
  ...
  $(window).keydown(function(e) {
    if (e.keyCode === 0 || e.keyCode === 32) {
      if ((BedJam.girl.lvl/scope.enemy.lvl)>success){
        battleText(BedJam.girl.name+' got away safely!');
        //back to overhead screen
        BedJam.game.paused = false;
      }
      else{
        battleText(BedJam.girl.name+' failed to get escape');
        scope.enemyAttack();
